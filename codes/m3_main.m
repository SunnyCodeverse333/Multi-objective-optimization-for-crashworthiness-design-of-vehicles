...
function f_p = objective(x,GPmodel,GPmodel_c, bestvalue)
    ...
    if predict(GPmodel_c , x) >=0
        P = 0; % penalty function coefficient 0 when constraint satisfied
    else
        P = 1; % penalty function when constraint violated
    end
    if bestvalue==inf
        f_p = -1+P;
        return;
    end
    [mew,sigma] = predict(GPmodel,x);
    
    z_star = (bestvalue-mew)/sigma;
    f_p = -normcdf(z_star) + P;
end
